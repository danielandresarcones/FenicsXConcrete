...
class concreteSlabExperiment(Experiment):
    ...
    def setup(self, bc='full'):
        ...
        if self.p.dim == 2:
            ...
            self.mesh = df.mesh.create_box(comm=MPI.COMM_WORLD,
                                           points=((0.0, 0.0), (self.p.length, self.p.breadth, self.p.height)), n=(
                self.p.num_elements_length, self.p.num_elements_breadth, self.p.num_elements_height),
    ...
    def create_displ_bcs(self, V):
        ...
        if self.p.dim == 2:
            ...
            displ_bcs.append(df.fem.dirichletbc(np.array([0, 0], dtype=ScalarType),
                                                df.fem.locate_dofs_geometrical(V, clamped_boundary), V))
            ...
            displ_bcs.append(df.fem.dirichletbc(np.array([0, 0, 0], dtype=ScalarType),
                                                df.fem.locate_dofs_geometrical(V, clamped_boundary), V))
